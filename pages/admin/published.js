...
const Blogs = () => {
  ...
  const toDraft = async () => {
    ...
    if (res.status === 200) {
      openToast("Posts moved to Draft!");
      fetchBlog();
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="relative ml-64 mr-5 2xl:w-10/12 3xl:w-8/12">
        ...
        <PageTitle title="Published" blogs={blogs} />
            ...
            <div>
              {!selectedItems.length ? (
                ...
                <div className="flex">
                  <div
                    onClick={toDraft}
                    className="flex items-center px-4 py-2 text-sm transition duration-300 rounded-full cursor-pointer hover:bg-black hover:text-white"
                  >
                    Revert to Draft
                  </div>
                  <div
            ...
            <table className="w-full font-avenir-light">
              <tbody id="blogTable">
                {blogs.map((blog, index) => (
                  <tr
                    key={blog.id}
                    className="border-t border-gray-200 group hover:bg-blue-50"
                  >
